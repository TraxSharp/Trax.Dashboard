...
public class NoFixedTaskDelayTests
{
    ...
    /// synchronise on the completion signal (TaskCompletionSource, polling) as in
    /// Trax.Docs/reference/test-conventions.md > Determinism, then delete the entry.
    /// </summary>
    ...
    public void TestSources_DoNotIntroduce_NewFixedDelays()
    {
        ...
        newOffenders
            .Should()
            .BeEmpty(
                "Trax.Docs/reference/test-conventions.md > Determinism forbids fixed-duration "
                    + "Task.Delay / Thread.Sleep in tests. "
